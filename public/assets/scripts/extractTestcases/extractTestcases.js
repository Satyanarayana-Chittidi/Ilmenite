...
const extractTestCases = async (tab, instanceId) => {
    try {
        ...
        if (result && result.result) {
            browserAPI.runtime.sendMessage({ testCase: result.result, instanceId }).catch(err => {
...
browserAPI.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.requestTestCases) {
        if (sender.tab) {
            extractTestCases(sender.tab, message.instanceId);
            sendResponse({ status: "extracting" });
        } else {
            browserAPI.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                const activeTab = tabs[0];
                if (activeTab) {
                    extractTestCases(activeTab, message.instanceId);
                    sendResponse({ status: "extracting" });
                }
            });
        }
